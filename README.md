...
- `backend/` — Python, FastAPI (API), SQLAlchemy async, PostgreSQL; backend modules are local onions. Orchestration coordination lives under `gamebuilder.orchestration.{domain,application,infrastructure,interfaces}`, while peer studio-team logic lives under `gamebuilder.team.<name>.{domain,application,infrastructure,interfaces}` as needed.
- `frontend/` — TypeScript, Next.js (App Router), Tailwind; prompt UI, artifact viewer, Phaser game runner.
...
Use **Docker Compose** for the database, Temporal, backend, and frontend. You do not need Python or Node installed on your machine for day-to-day development via Compose.
...
- **Backend**: from `backend/`, with [uv](https://github.com/astral-sh/uv): `uv sync` then `uv run uvicorn gamebuilder.main:app --reload --port 8080` (Postgres and Temporal must be reachable; Compose can run just `db` + `temporal`).
- **Frontend**: from `frontend/`, `npm install` then `npm run dev` with `NEXT_PUBLIC_API_BASE_URL=http://localhost:8080`.
